...
# Funciones de Streamlit (las mismas que en tu app5.py)
def show_header():
    st.markdown(f"""
    <div class="header">
...
def generate_pdf_report(empresa, estadisticas, preguntas, observaciones_generales, logo_empresa=None, logo_sesaco=None):
    """Genera un PDF del reporte con estructura profesional"""
    ...
    verde_bosque = (0, 107, 63)   # RGB para #006b3f
    verde_hierba = (107, 190, 68)  # RGB para #6bbe44
    gris_claro = (242, 242, 242)   # RGB para #f2f2f2
    ...
    if logo_sesaco:
        try:
            # Convertir a imagen PIL y guardar temporalmente
            img = Image.open(BytesIO(logo_sesaco.getvalue()))
            with tempfile.NamedTemporaryFile(delete=False, suffix='.png') as tmp_file:
                img.save(tmp_file, format='PNG')
                tmp_path = tmp_file.name
            pdf.image(tmp_path, x=10, y=8, w=30)
            os.unlink(tmp_path)
        except Exception as e:
            st.error(f"Error al procesar logo SESACO: {str(e)}")
    ...
    if logo_empresa:
        try:
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix='.png') as tmp_file:
                img.save(tmp_file, format='PNG')
                tmp_path = tmp_file.name
            pdf.image(tmp_path, x=170, y=8, w=30)
            os.unlink(tmp_path)
        except Exception as e:
            st.error(f"Error al procesar logo empresa: {str(e)}")
            pdf.set_font("Arial", 'B', 10)
            pdf.set_text_color(*verde_bosque)
    ...
    pdf.set_text_color(*verde_bosque)
    pdf.cell(0, 10, "INFORME DE VERIFICACIÓN SST", ln=1, align='C')
    pdf.set_font("Arial", 'B', 14)
    pdf.cell(0, 8, empresa['razon_social'], ln=1, align='C')
    pdf.cell(0, 8, f"RUC: {empresa['ruc']}", ln=1, align='C')
    pdf.ln(10)
    
    # --- TABLA DE INFORMACIÓN GENERAL ---
    pdf.set_font("Arial", 'B', 12)
    pdf.set_text_color(*verde_bosque)
    pdf.cell(0, 8, "INFORMACIÓN GENERAL", ln=1)
    ...
    info_data = [
        ...
        ["Total Trabajadores", empresa['total_trabajadores']],
        ["Tipo de Empresa", empresa['tipo']],
        ["Inspector", st.session_state.user_info['nombre']],
        ["Cédula Inspector", st.session_state.user_info['cedula']]
    ]
    ...
    pdf.cell(0, 8, "Valor", border=1, fill=True, ln=1)
    pdf.set_text_color(0, 0, 0)

    for item in info_data:
        pdf.cell(60, 8, item[0], border=1)
        pdf.multi_cell(0, 8, str(item[1] if item[1] else "N/A"), border=1)
    ...
    pdf.set_font("Arial", 'B', 12)
    pdf.set_text_color(*verde_bosque)
    pdf.cell(0, 8, "ANTECEDENTES", ln=1)
    pdf.set_text_color(0, 0, 0)
    pdf.set_font("Arial", size=10)
    
    antecedentes = """
    El presente informe muestra los resultados de la verificación de cumplimiento de normativas de 
    Seguridad y Salud en el Trabajo realizada por SESACO Seguridad Industrial S.A., en cumplimiento 
    con lo establecido en el Acuerdo Ministerial 196 (2024) y normativa conexa.
    
    El objetivo de esta verificación es evaluar el grado de cumplimiento de la empresa con las 
    disposiciones legales en materia de seguridad y salud ocupacional, identificando áreas de 
    conformidad y oportunidades de mejora para garantizar ambientes de trabajo seguros y saludables.
    """
    
    pdf.multi_cell(0, 5, antecedentes)
    pdf.ln(5)
    
    # --- MARCO LEGAL ---
    pdf.set_font("Arial", 'B', 12)
    pdf.set_text_color(*verde_bosque)
    pdf.cell(0, 8, "MARCO LEGAL APLICABLE", ln=1)
    pdf.set_text_color(0, 0, 0)
    pdf.set_font("Arial", size=10)
    
    marco_legal = """
    La verificación se realizó considerando el siguiente marco normativo:
    
    - Acuerdo Ministerial 196 (2024) - Reglamento de Seguridad y Salud en el Trabajo
    - Código del Trabajo (2005) y sus reformas
    - Decreto Ejecutivo 255 (2024) - Reglamento de Seguridad Industrial
    - Resolución 957 (2008) - Normas para la gestión de seguridad y salud ocupacional
    - NTE INEN-ISO 3864-1 - Señalización de seguridad
    - Otras normativas específicas según actividad económica
    """
    
    pdf.multi_cell(0, 5, marco_legal)
    pdf.ln(10)
    ...
    pdf.set_text_color(255, 255, 255)
    pdf.cell(0, 8, f"ESTADO GENERAL: {estado} ({cumplimiento:.1f}%)", border=1, ln=1, fill=True, align='C')
    pdf.set_text_color(0, 0, 0)
    ...
    try:
        ...
        pdf.ln(5)
        pdf.cell(0, 5, "Figura 1: Distribución general de cumplimiento", ln=1, align='C')
        os.unlink(tmp_path)
        
    except Exception as e:
        pdf.multi_cell(0, 5, f"Error al generar gráfico: {str(e)}")
    ...
    pdf.set_font("Arial", 'B', 12)
    pdf.set_text_color(*verde_bosque)
    pdf.cell(0, 8, "ESTADÍSTICAS DETALLADAS", ln=1)
    pdf.set_text_color(0, 0, 0)
    ...
    try:
        ...
        bars = ax_bar.barh(secciones, porcentajes, color=verde_hierba)
        ...
        pdf.ln(5)
        pdf.cell(0, 5, "Figura 2: Cumplimiento por área de verificación", ln=1, align='C')
        os.unlink(tmp_path)
    ...
    pdf.cell(30, 8, "% CUMPL.", border=1, fill=True, ln=1)
    pdf.set_text_color(0, 0, 0)
    ...
    for seccion, datos in estadisticas.get("secciones", {}).items():
        ...
        pdf.cell(30, 8, f"{porcentaje:.1f}%", border=1, ln=1)
    ...
    pdf.set_font("Arial", 'B', 12)
    pdf.set_text_color(*verde_bosque)
    pdf.cell(0, 8, "DETALLE POR SECCIÓN", ln=1)
    pdf.set_text_color(0, 0, 0)
    
    for seccion, datos in estadisticas.get("secciones", {}).items():
        # Nueva página para cada sección después de la primera
        ...
        pdf.set_font("Arial", 'B', 12)
        pdf.set_text_color(*verde_bosque)
        pdf.cell(0, 8, seccion.replace("_", " ").upper(), ln=1)
        ...
        # Estadísticas de la sección
        pdf.set_font("Arial", 'B', 10)
        pdf.cell(0, 8, f"Porcentaje de cumplimiento: {porcentaje:.1f}%", ln=1)
        
        # Tabla de estadísticas
        pdf.set_fill_color(*verde_bosque)
        pdf.set_text_color(255, 255, 255)
        pdf.cell(60, 8, "INDICADOR", border=1, fill=True)
        pdf.cell(30, 8, "VALOR", border=1, fill=True, ln=1)
        pdf.set_text_color(0, 0, 0)
        
        pdf.cell(60, 8, "Total de ítems", border=1)
        pdf.cell(30, 8, str(datos.get("total", 0)), border=1, ln=1)
        
        pdf.cell(60, 8, "No aplica", border=1)
        pdf.cell(30, 8, str(datos.get("no_aplica", 0)), border=1, ln=1)
        
        pdf.cell(60, 8, "Ítems evaluados", border=1)
        pdf.cell(30, 8, str(total_aplicable), border=1, ln=1)
        
        pdf.cell(60, 8, "Cumple", border=1)
        pdf.cell(30, 8, str(datos.get("cumple", 0)), border=1, ln=1)
        
        pdf.cell(60, 8, "No cumple", border=1)
        pdf.cell(30, 8, str(datos.get("no_cumple", 0)), border=1, ln=1)
        ...
        if preguntas_no_cumplen:
            pdf.set_font("Arial", 'B', 10)
            pdf.cell(0, 8, f"No conformidades encontradas ({len(preguntas_no_cumplen)}):", ln=1)
    ...
    pdf.set_font("Arial", 'B', 12)
    pdf.set_text_color(*verde_bosque)
    pdf.cell(0, 8, "OBSERVACIONES GENERALES", ln=1)
    pdf.set_text_color(0, 0, 0)
    pdf.set_font("Arial", size=10)
    ...
    pdf.set_font("Arial", 'B', 12)
    pdf.set_text_color(*verde_bosque)
    pdf.cell(0, 8, "FIRMA Y SELLO DEL INSPECTOR", ln=1)
    pdf.set_text_color(0, 0, 0)
    pdf.ln(20)
    
    # Línea para firma
    pdf.cell(80, 8, f"Nombre: {st.session_state.user_info.get('nombre', '')}", ln=1)
    pdf.cell(80, 8, "Cédula: _________________________", ln=1)
    pdf.cell(80, 8, "Firma:  _________________________", ln=1)
    pdf.ln(20)
    ...
    # Línea divisoria
    pdf.set_draw_color(*verde_bosque)
    ...
    if 'empresa_actual' in st.session_state and st.session_state.empresa_actual:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                observaciones_generales = st.text_area("Observaciones General:", 
